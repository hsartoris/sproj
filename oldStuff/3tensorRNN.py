...
class seqData(object):
	def __init__(self, minIdx = 0, maxIdx=500):
		...
		while randIdx < maxIdx and simpIdx < maxIdx:
			if np.random.rand() < .5:
				temp = np.loadtxt(prefix + "/simplicial/spikes/" + str(simpIdx) + ".csv", delimiter=',').transpose()
				self.labels.append([1,0])
				simpIdx += 1
			else:
				temp = np.loadtxt(prefix + "/random/spikes/" + str(randIdx) + ".csv", delimiter=',').transpose()
	...
	def pad(self):
		global maxLen
		for i in range(len(self.data)):
			self.data[i] = np.append(self.data[i], np.array([[0,0]]*(maxLen-len(self.data[i]))), axis=0)
