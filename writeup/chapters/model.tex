...
where $\mathbb{O}$ is the output matrix and $\mathbb{R}^0$ is an 
\textit{n}-vector wherein each element has been randomly assigned 0 or 1, based 
on the spike rate of the simulation:
\[
...
transformation is demonstrated in figure \ref{fig:transform}.
