...
describing the generator, in order to provide a target for the model to converge 
towards.
...
\[ \begin{bmatrix}
	...
	\end{bmatrix} \]
Applying this process to the data in \figref{fig:exoutput}, including 
...
Finally, an elementwise activation function\footnote{SEE NN PRINCIPLES} is 
applied to the matrix outputs each layer (see \ref{sec:activation}).
...
$\mathbb{W}_{tot}^\prime$, are trained on by the optimizer\footnote{SEE 
