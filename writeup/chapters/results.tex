...
\figref{fig:9neur_loss+params}. The results demonstrate that, while the 
locality-based approach is able to reconstruct networks, it does not offer 
substantive improvement over the much more straightforward benchmark model, at 
least in the cases that we have considered. An example run can be found in 
\figref{fig:samepred}.
\begin{figure}[h]
	\centering
	\hfill
...
This does not, however, invalidate the number of other advantages offered by our 
architecture.
...
and fed into the model described in \ref{results_3neur}:
