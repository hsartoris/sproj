...
class Model():
    def __init__(self, b, d, n, data, labels, batchSize, structure=[1,1], 
        ...
        if dumb:
            self.weights['layer1'] = tf.Variable(tf.random_normal([d,d],
                stddev=weights_stddev))
        else:
            self.weights['layer1'] = [ tf.Variable(tf.random_normal([d,2*d],
                stddev=weights_stddev)), tf.Variable(tf.random_normal([d,2*d],
                stddev=weights_stddev)), self.weights['layer1'][2]]
    ...
    def layer1dumb(self):
        return self.activation(tf.add(tf.einsum('ij,kjl->kil', 
            self.weights['layer1'], self.layer0_1), tf.tile(self.biases['layer1'],
    ...
    def layer1(self):
        ...
        out_total = tf.concat([vert, data], 1)
