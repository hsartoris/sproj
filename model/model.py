...
class Model():
    ...
    def layer0(self, x):
        ...
        return tf.nn.relu(tf.add(tf.einsum('ij,kjl->kil', self.weights['layer0'], total),
                tf.tile(self.biases['layer0'], [x.get_shape()[0],1,self.n*self.n])))

    @lazy_property
    def layer1(self, x):
        a_total = tf.concat([tf.einsum('ijk,kl->ijl', 
            tf.einsum('ijk,kl->ijl', x, tf.transpose(self.expand)), self.expand), x], 1)
        ...
        return tf.nn.relu(tf.add(tf.add(a_total, b_total),
                tf.tile(self.biases['layer1'], [x.get_shape()[0],1,self.n*self.n])))
    
    @lazy_property
    def layerFinal(self, x):
        return tf.einsum('ij,ljk->lik', self.weights['final'], x)
