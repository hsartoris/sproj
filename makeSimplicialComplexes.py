...
for i in range(numGraphs):
	...
	while np.sum(matrix) < percent * maxEdges:
		...
		sc.map(matrix, rules)
		complexes += 1
	if verbosity > 0: print("Completed graph " + str(i) + " with " + str(complexes) + " simplicial complexes, and " + str(np.sum(matrix)) + " edges (" + str(float(np.sum(matrix))/maxEdges) + "% connected)")
	np.savetxt(prefix + str(i) + ".csv", matrix, delimiter=',', fmt='%i')
	subprocess.call(["pipeline/pipe.py", prefix + str(i) + ".csv", str(timesteps), prefix])
	# haha I love names
#	spikes = pipeline.pipe.pipe(matrix, timesteps)
#	np.savetxt(prefix + "/spikes/" + str(i) + "_spikes.csv", spikes, delimiter=',', fmt='%i')
